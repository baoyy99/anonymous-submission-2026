export CUDA_VISIBLE_DEVICES=0
#!/bin/bash

# loss="mse"
# loss="mae"
loss="pall_ema_tqa_mae"
# loss="pall_ema_tqa_mse"

seq_len=96
label_len=48
features="M"
embed="timeF"
target="OT"
freq="h"
train_epochs=10
patience=3

dataset_name=("ETTh1" "ETTh2" "ETTm1" "ETTm2" "traffic" "weather" "solar"  "electricity"  "01data")
root_path=("./dataset2026/ETT-small"
    "./dataset2026/ETT-small"
    "./dataset2026/ETT-small"
    "./dataset2026/ETT-small"
    "./dataset2026/traffic"
    "./dataset2026/weather"
    "./dataset2026/Solar"
    "./dataset2026/electricity"
    "./dataset2026/01data"
)
data_path=("ETTh1.csv"
    "ETTh2.csv"
    "ETTm1.csv"
    "ETTm2.csv"
    "traffic.csv"
    "weather.csv"
    "solar_AL.txt"
    "electricity.csv"
    "01data.csv"
)
data_type=("ETTh1"
    "ETTh2"
    "ETTm1"
    "ETTm2"
    "custom"
    "custom"
    "Solar"
    "custom"
    "custom"
)
enc_in=(7 7 7 7 862 21 137 321 10)
dec_in=(7 7 7 7 862 21 137 321 10)
c_out=(7 7 7 7 862 21 137 321 10)
batch_size=(32 32 32 32 16 32 32 32 32)
learning_rate=(0.0001 0.0001 0.0001 0.0001 0.001 0.0001 0.0001 0.0001 0.0001)
lradj=('type3' 'type3' 'type3' 'type3' 'sigmoid' 'sigmoid' 'sigmoid' 'sigmoid' 'type1')
cycle=(24 24 96 96 168 144 144 168 24)
use_revin=(1 1 1 1 1 1 0 1 1)

# for i in {0..7}
# for i in {0..4}
# for i in {5..7}
for i in 8
do
#   for pred_len in 192 336 720
  for pred_len in 96
  do
    for TQALoss_alpha in 0 1
    do
        for TQALoss_beta in 0 1
        do
        #   for TQALoss_q in 0.51 0.7 0.9
        #   do
                echo "----- ${dataset_name[$i]} ${pred_len} -----"
                python -u 10-TQNet-master/run.py \
                    --is_training 1 \
                    --root_path ${root_path[$i]} \
                    --data_path ${data_path[$i]} \
                    --model TQNet \
                    --data ${data_type[$i]} \
                    --features $features \
                    --seq_len $seq_len \
                    --pred_len $pred_len \
                    --enc_in ${enc_in[$i]} \
                    --des 'Exp' \
                    --itr 1 \
                    --batch_size ${batch_size[$i]} \
                    --learning_rate ${learning_rate[$i]} \
                    --cycle ${cycle[$i]} \
                    --train_epochs ${train_epochs} \
                    --patience ${patience} \
                    --use_revin ${use_revin[$i]} \
                    --loss $loss\
                    --TQALoss_alpha $TQALoss_alpha\
                    --TQALoss_beta $TQALoss_beta\
                    --TQALoss_gamma 0.1\
                    --TQALoss_q 0.8
           
    done
done
done
done
# done
#  --TQALoss_q $TQALoss_q